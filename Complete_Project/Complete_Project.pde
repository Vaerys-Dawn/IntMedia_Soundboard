...
import java.util.Random;
  ...
  public void setup() {
    ...
    float increment = 0;
    try {
      effects.put(49, new FlameEffect("fire.mp3", 1, increment += effectWidth, effectWidth));          // 1
      effects.put(50, new SineWaveEffect("sinewave.pm3", 2, increment += effectWidth, effectWidth));   // 2
      effects.put(51, new ZigZagEffect("", 3, increment += effectWidth, effectWidth));                 // 3
      effects.put(52, new ZigZagEffect("", 3, increment += effectWidth, effectWidth));                 // 3
      effects.put(53, new ZigZagEffect("", 3, increment += effectWidth, effectWidth));                 // 3
